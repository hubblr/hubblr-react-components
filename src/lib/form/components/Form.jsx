...
class Form extends React.Component {
  constructor(props) {
    ...
    this.getComponentsToJumpToEmptyFields =
      this.getComponentsToJumpToEmptyFields.bind(this);
    this.getComponentsToJumpToForErrors =
      this.getComponentsToJumpToForErrors.bind(this);
    this.registerComponentFilledCheck =
      this.registerComponentFilledCheck.bind(this);
    this.unregisterComponentFilledCheck =
      this.unregisterComponentFilledCheck.bind(this);
    this.triggerComponentFilledChecks =
      this.triggerComponentFilledChecks.bind(this);
    this.registerComponentValidator =
      this.registerComponentValidator.bind(this);
    this.unregisterComponentValidator =
      this.unregisterComponentValidator.bind(this);
    ...
    this.updateErrorMessageForComponent =
      this.updateErrorMessageForComponent.bind(this);
    this.triggerComponentValidators =
      this.triggerComponentValidators.bind(this);
    this.testAllComponentValidators =
      this.testAllComponentValidators.bind(this);
  ...
  getComponentsToJumpToEmptyFields() {
    ...
    return this.getFilteredComponentsToJumpTo((groupName) =>
      this.emptyFields.includes(groupName)
    );
  }
  ...
  markComponentGroupTouched(groupName, touched) {
    this.setState((prevState) => {
      const { [groupName]: previousErrorOfField, ...others } =
        prevState.componentGroupsTouched;
  ...
  testAllComponentValidators() {
    const validationResults = Object.keys(this.componentValidators).map(
      (componentName) => {
  ...
  updateErrorMessageForComponent(fieldName, error = null) {
    this.setState((prevState) => {
      const { [fieldName]: previousErrorOfField, ...others } =
  ...
  triggerComponentFilledChecks(fieldNames) {
    const emptyFields = [];
    Object.entries(this.componentFilledChecks).forEach(
      ([fieldName, fillCheck]) => {
  ...
  render() {
    ...
    const { onSubmit, children, onSubmitValidationFailed, className } =
      this.props;
    const { validationErrors } = this.state;
    const { componentGroupsTouched } = this.state;
